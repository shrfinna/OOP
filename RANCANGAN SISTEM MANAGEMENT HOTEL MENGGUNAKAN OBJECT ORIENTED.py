...
class Hotel: 
    def __init__(self, root):
        ...
        TopFrame = Frame(MainFrame, bd=14, width=1350, height=550, padx=20, relief=RIDGE, bg="powder blue")
        TopFrame.pack(side=TOP)
        LeftFrame = Frame(TopFrame, bd=10, width=450, height=550, padx=2, relief=RIDGE, bg="powder blue")
        LeftFrame.pack(side=LEFT)
        RightFrame = Frame(TopFrame, bd=10, width=820, height=550, padx=2, relief=RIDGE, bg="powder blue")
        RightFrame.pack(side=RIGHT)
        BottomFrame = Frame(MainFrame, bd=10, width=350, height=550, padx=20, relief=RIDGE, bg="powder blue")
        BottomFrame.pack(side=BOTTOM)
#=======================================================================================================================#
        def iExit():
            iExit = tkinter.messagebox.askyesno("Hotel Management Systems", "Confrim if you want to exit")
            if iExit > 0:
                root.destroy()
                return 
        ...
        def Receipt():
            ...
            "\t"+Adderss.get()+"\t"+PostCode.get()+"\t"+Mobile.get()+"\t"+Nationality.get()+"\t"+CheckInDate.get()+
        ...
        def TotalCostandDate():
            ...
            if (Meal.get() == "Breakfast" and RoomType.get() == "Single"):
                ...
                q4=float(q1+q2)
                q5=float(q3*q4)
                Tax="Rp" + str('%.2f'%((q5)*0.09))
                ST="Rp" + str('%.2f'%((q5)))
                TT=  "Rp" + str('%.2f'%(q5 + ((q5)*0.09)))
                PaidTax.set((Tax))
                SubTotal.set(ST)
                TotalCost.set(TT)
        ...
        self.lblAddress = Label(LeftFrame, font =('arial', 12, 'bold'), text="Adderss:", padx= 2, bg="powder blue")
        self.lblAddress.grid(row=3, column=0, sticky=W)
        self.txtAdderss = Entry(LeftFrame, font =('arial', 12, 'bold'), textvariable=Adderss, width=20)
        self.txtAdderss.grid(row=3, column=1, pady=3, padx=20)
        ...
        self.cboNationality['value'] = ('', 'British', 'Nigeria', 'Malaysia', 'Indonesia', 'Australia', 'Itali', 'Canada')
        self.cboNationality.current(0)
        self.cboNationality.grid(row=7, column=1, pady=3, padx=20)

        self.lblDOB = Label(LeftFrame, font =('arial', 12, 'bold'), text="Date of Bitrh:", padx= 2, bg="powder blue")
        ...
        self.lblRoomExit = Label(LeftFrame, font =('arial', 12, 'bold'), text="Room Exit:", padx= 2, bg="powder blue")
        self.lblRoomExit.grid(row=16, column=0, sticky=W)
        self.cboRoomExit = ttk.Combobox(LeftFrame, textvariable=RoomExit, state='readonly',font =('arial', 12, 'bold'), width=18)
        self.cboRoomExit['value'] = ('', '101', '102', '103', '104', '105', '106', '107', '108', '109', '110')
        self.cboRoomExit.current(0)
        self.cboRoomExit.grid(row=16, column=1, pady=3, padx=20)
...
#Receipt==========================================================================================================================#
        ...
        self.lblLabel.grid(row=0, columnspan=17)

        self.txtReceipt = Text(RightFrame, height=15, width=108, font=('arial', 11, 'bold'))
...
#==========================================================================================================================#
        ...
        self.btnReceipt = Button(BottomFrame, padx=16, pady=1, bd=4, fg="black", font=('arial', 16, 'bold'),
                        width=21, height=2, bg="powder blue", text="Receipt ", command=Receipt).grid(row=0, column=5, padx=4)
        self.btnReset = Button(BottomFrame, padx=16, pady=1, bd=4, fg="black", font=('arial', 16, 'bold'),
                        width=21, height=2, bg="powder blue", text="Reset ", command=Reset).grid(row=0, column=6, padx=4)
        self.btnExit = Button(BottomFrame, padx=16, pady=1, bd=4, fg="black", font=('arial', 16, 'bold'),
                        width=21, height=2, bg="powder blue", text="Exit ", command=iExit).grid(row=0, column=7, padx=4)
